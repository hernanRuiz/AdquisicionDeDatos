...
public class Energon extends JPanel implements KeyListener {
	...
	public BranchGroup createSceneGraph() {
		...
		QuadArray lsa = new QuadArray(48, QuadArray.COORDINATES
				| QuadArray.NORMALS);
		...
		light1.setInfluencingBounds(new BoundingSphere(new Point3d(-5.0, 0, 0),
				10.0));
		light1.setColor(new Color3f(1f, 1f, 1f));
		light1.setDirection(new Vector3f(0, 1, 0));
		transformGroup.addChild(light1);
		DirectionalLight light2 = new DirectionalLight();
		light2.setInfluencingBounds(new BoundingSphere(new Point3d(5.0, 0, 0),
				10.0));
		...
		f1.setSchedulingBounds(new BoundingBox());
		f1.setTransformGroup(transformGroup);
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyChar() == 's') {
			...
			transformGroup.getTransform(transVieja);
			Matrix4d matriz = new Matrix4d();
			
			transVieja.get(matriz);
			
			transVieja.rotX(xLoc);
		...
		if (e.getKeyChar() == 'a') {
			...
			//Mover linealmente
//			transform3D.setTranslation(new Vector3f(xLoc, 0f, 0f));
